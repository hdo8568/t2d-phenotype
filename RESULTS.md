...
| parse failures (all recovered by retry) | 0 | a response hit the token ceiling mid-JSON; the retry succeeded |
| parse failures (unrecovered) | **0** | never parsed, raw text in `parse_failures.log` |
...
> **69 of 3,539 adjudicated patients (1.9%) diverge from the rule baseline** — 68 negative->positive, 1 positive->negative.

## Method

- Model: `claude-sonnet-5`, one call per patient, reasoning effort `medium`.
- Mode: **full cohort (--all)** — 3,539 of 3,539 patients adjudicated by the LLM; the remainder carry the rule label.
...
- Concurrency: started at 16 workers, never throttled — finished at 16.
...
| metric | value |
| --- | --- |
| wall clock | **0.31 min** (final pass only; 3,539 patients were labeled in an earlier pass and resumed from checkpoint) |
| API calls | 3,588 |
| input tokens | 8,225,206 |
| output tokens | 485,152 |
| cache write tokens | 0 |
| cache read tokens | 0 |
| **total cost** | **$21.3019** |
| cost per patient | $0.00602 |
| mean latency | 10.29 s |
| median latency | 3.49 s |
| p95 latency | 63.29 s |
| retries | 0 |
| fallbacks | 0 |
...
- Patients adjudicated by the LLM: **3,539**
- Labels matching the rule baseline: **3,470**
- Divergent from the PheKB rule label: **69** (1.9% of adjudicated)
  - rules negative -> LLM positive: **68**
  - rules positive -> LLM negative: **1**
- Of the rule-negative flips, **68** carry a T2D
  diagnosis code and 0 do not.

Final submission: **381 positive** / 3,158 negative out of 3,539.
